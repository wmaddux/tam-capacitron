...
- **Phase 2 UI:** Three-column layout (Inputs | Outputs | Definition). **Cluster card:** Server instance specs (vCPUs, RAM (GB), Storage, Networking) in a box—only RAM affects calculations; then topology (nodes, devices, device size), overhead %, divider, Nodes lost. Namespaces cards with Add/Remove, param help in Definition column. **Show my work:** Storage, Memory, and Failure sections with step-by-step formulas. State is `{ cluster, namespaces[] }`; cluster includes `vcpus`, `instance_storage`, `instance_networking`. Load from defaults, Load from collectinfo, Export. See [MANIFEST.md](MANIFEST.md) and [app/static/index.html](app/static/index.html).
