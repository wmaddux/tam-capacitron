...
- **Multi-namespace:** Implemented. Engine `run_multi(cluster, namespaces)`; API **POST /api/compute-v2**; ingest returns `cluster` + `namespaces` (with per-namespace `storage_pattern`, `placement` when derivable). Load from collectinfo: .zip/.tgz/.tar; Device Total TB/GB; no-device namespaces get In-Memory (MMM), 0 device storage. See [docs/API_MULTI_NAMESPACE.md](docs/API_MULTI_NAMESPACE.md).
- **Config-driven defaults and sliders:** [config/inputs.json](config/inputs.json) holds defaults and slider min/max/step; [app/config.py](app/config.py) loads it; **GET /api/input-config** returns both; frontend fetches on load; `get_default_inputs()` and Pydantic use config. Restart app after editing config.
- **Phase 2 UI:** Three-column layout: columns 1 and 2 fixed 480px, column 3 flexible; stacks below 1100px. **Cluster card:** Server instance specs (vCPUs, RAM (GB), Storage, Networking) in a box—only RAM affects calculations; then topology (nodes, devices, device size); then **Overhead & performance** subsection (Overhead %, IOPS per disk (K), Throughput per disk (MB/s)—defaults 320/1500); divider; Nodes lost, Data growth. Namespaces cards with Add/Remove, param help in column 3. **Show my work:** Storage, Memory, and Failure sections with step-by-step formulas. State is `{ cluster, namespaces[] }`; cluster includes `vcpus`, `instance_storage`, `instance_networking`, `iops_per_disk_k`, `throughput_per_disk_mbs`. Load from defaults, Load from collectinfo (fills cluster name and sets IOPS/Throughput to 320/1500), Export. See [MANIFEST.md](MANIFEST.md) and [app/static/index.html](app/static/index.html).
- **Calculations:** [docs/CALCULATIONS.md](docs/CALCULATIONS.md) is the source of truth. Memory: Total memory used base = **Primary Index Shmem** (64 B per replicated object) + **Secondary Index Shmem** (SI cushion 16 MiB). See [docs/CALCULATION_CATALOG.md](docs/CALCULATION_CATALOG.md) and `core/formulas.py`.
