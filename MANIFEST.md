...
**Current scope:** Multi-namespace supported. UI has Cluster card with **Server instance specs** (vCPUs, RAM (GB), Storage, Networking in a box—for mapping cloud instance types; only RAM affects calculations), default storage pattern, then **topology** (Nodes per cluster, Devices per node, Device size (GB)), then **Overhead & performance** subsection (Overhead %, IOPS per disk (K), Throughput per disk (MB/s)—defaults 320 and 1500; Throughput slider max 3000), then divider and **Resilience** (Nodes lost, Data growth (%/year)). Namespaces cards (Add/Remove) with storage pattern (HMA/In-Memory/All Flash/DMD/Custom), Custom placement, compression, and capacity thresholds. State includes `cluster.vcpus`, `cluster.instance_storage`, `cluster.instance_networking`. **Defaults and sliders** come from [config/inputs.json](config/inputs.json); UI fetches **GET /api/input-config** on load. **Layout:** Columns 1 and 2 fixed 480px, column 3 flexible; stacks to one column below 1100px. Column 3: Parameter help (click param label) + **Show my work** (Storage, Memory, and Failure sections with step-by-step formulas and separators). Outputs: numbers with thousand separators; Storage shows Data stored (TB) and Device total storage (TB); redundant Total available storage row removed. Compute uses **POST /api/compute-v2**. [docs/CALCULATIONS.md](docs/CALCULATIONS.md) is the source of truth for capacity formulas; SI cushion 16 MiB. **Collectinfo load:** Bundles .zip/.tgz/.tar; **Cluster name** from Cluster Summary when present; **IOPS/Throughput** set to fixed 320 K and 1500 MB/s (mapping layer); Device Total TB/GB; no-device namespaces get In-Memory (MMM), 0 device storage; RAM derived from system_free_mem when summary has no Memory Total (7.x). Derivation script [tests/test_collectinfo_derivations.sh](tests/test_collectinfo_derivations.sh); [scripts/gather_asadm_for_calculations.sh](scripts/gather_asadm_for_calculations.sh).
...
| **UI reference** | [docs/mockups/](docs/mockups/) | **Current app** ([app/static/index.html](app/static/index.html)): three-column layout—columns 1 & 2 fixed 480px, column 3 flexible; stacks below 1100px. Cluster: Server instance specs box, then topology (nodes, devices, device size), then **Overhead & performance** subsection (Overhead %, IOPS per disk (K), Throughput per disk (MB/s)), then Nodes lost and Data growth. Namespaces: pattern pills, Custom placement, compression, capacity thresholds. Show my work: Storage, Memory, Failure sections. Param help on label click; Load from collectinfo fills cluster name and sets IOPS/Throughput to 320/1500. |
...
| Area | Path | Notes |
|------|------|-------|
| Input/output model | [core/model.py](core/model.py) | `ClusterInputs` (includes `vcpus`, `instance_storage`, `instance_networking`; `iops_per_disk_k` default 320, `throughput_per_disk_mbs` default 1500), `NamespaceInputs`, `CapacityInputs`, `CapacityOutputs`; `get_default_inputs()` from config |
...
- Collectinfo ingestion returns `cluster` + `namespaces` (and legacy flat). Cluster includes `cluster_name` (from summary "Cluster Name") and fixed `iops_per_disk_k` 320, `throughput_per_disk_mbs` 1500. Load accepts .zip, .tgz, .tar. No-device namespaces get In-Memory (MMM), avg_record_size 0. RAM derived from system_free_mem when summary has no Memory Total. Sample bundle: `bundles/fidelity-case00044090-20250226.zip` (when present).
